...
class LogProbsFromLogitsAndActionsTest(tf.test.TestCase, parameterized.TestCase):
...
class VtraceTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_vtrace_from_logits(self, batch_size):
        ...
        values = {
            "behaviour_policy_logits": _shaped_arange(seq_len, batch_size, num_actions),
        ...
        with self.test_session() as session:
            from_logits_output_v = session.run(from_logits_output, feed_dict=feed_dict)
            (
                ground_truth_log_rhos,
                ground_truth_behaviour_action_log_probs,
                ground_truth_target_action_log_probs,
            ) = session.run(ground_truth, feed_dict=feed_dict)
        ...
        self.assertAllClose(from_iw_v.pg_advantages, from_logits_output_v.pg_advantages)
        self.assertAllClose(
            ground_truth_behaviour_action_log_probs,
