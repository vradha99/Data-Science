...
class DynamicBatchingTest(tf.test.TestCase):
    ...
    def test_input_batch_size_should_be_one(self):
        with self.test_session() as session:
            ...
            with self.assertRaisesRegexp(
                tf.errors.InvalidArgumentError, "requires batch size 1"
            ):
                coord.join()
    ...
    def test_minimum_batch_size(self):
        with self.test_session() as session:
            ...
            self.assertLessEqual(0.9, duration.total_seconds())
            ...
            # added.
            self.assertGreaterEqual(0.5, duration.total_seconds())
    ...
    def test_static_shape(self):
        ...
        @dynamic_batching.batch_fn_with_options(
            minimum_batch_size=2, maximum_batch_size=2, timeout_ms=None
        )
        def f2(a):
    ...
    def test_invalid_computation_id(self):
        with self.test_session() as session:
            ...
            with self.assertRaisesRegexp(
                tf.errors.InvalidArgumentError, "Invalid computation id"
            ):
